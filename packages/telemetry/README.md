...
```bash
npm install @pi-chorus/telemetry
```
...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from '@pi-chorus/telemetry';
...
```typescript
import { InMemoryTelemetryContext } from '@pi-chorus/telemetry';
...
`@pi-chorus/telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from '@pi-chorus/telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from '@pi-chorus/agent-core';
...
- `@pi-chorus/telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `@pi-chorus/ai` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `@pi-chorus/agent-core` owns and exports the pi AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
